...
void test()
{
	...
	router0->outputDiagram(IMAGE_OUTPUT_PATH "output/treeRootCrash01-1");
	...
	newanddeletedobjects_0 = router0->newAndDeletedObjectListsFromHyperedgeImprovement();
	router0->outputDiagram(IMAGE_OUTPUT_PATH "output/treeRootCrash01-2");
	router0->processTransaction();

	newanddeletedobjects_0 = router0->newAndDeletedObjectListsFromHyperedgeImprovement();
	router0->outputDiagram(IMAGE_OUTPUT_PATH "output/treeRootCrash01-3");
	
	router0->hyperedgeRerouter()->registerHyperedgeForRerouting(junction0_526491597);
	router0->processTransaction();
	router0->outputDiagram(IMAGE_OUTPUT_PATH "output/treeRootCrash01-4");
