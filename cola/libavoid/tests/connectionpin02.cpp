...
int main(void)
{
    ...
    router->outputDiagram(IMAGE_OUTPUT_PATH "output/connectionpin02-1");

    router->moveShape(shapeRef1, 0, 90);
    router->moveShape(shapeRef2, 0, -90);

    router->processTransaction();
    router->outputDiagram(IMAGE_OUTPUT_PATH "output/connectionpin02-2");
    
    router->moveShape(shapeRef3, 0, -60);

    router->processTransaction();
    router->outputDiagram(IMAGE_OUTPUT_PATH "output/connectionpin02-3");
    ...
    */
    ...
    router->processTransaction();
    router->outputDiagram(IMAGE_OUTPUT_PATH "output/connectionpin02-4");
    
    shapeRef3->transformConnectionPinPositions(Avoid::TransformationType_CW180);
    router->processTransaction();
    router->outputDiagram(IMAGE_OUTPUT_PATH "output/connectionpin02-5");

    shapeRef3->transformConnectionPinPositions(Avoid::TransformationType_CW270);
    router->processTransaction();
    router->outputDiagram(IMAGE_OUTPUT_PATH "output/connectionpin02-6");

    shapeRef3->transformConnectionPinPositions(Avoid::TransformationType_FlipX);
    router->processTransaction();
    router->outputDiagram(IMAGE_OUTPUT_PATH "output/connectionpin02-7");

    shapeRef3->transformConnectionPinPositions(Avoid::TransformationType_FlipY);
    router->processTransaction();
    router->outputDiagram(IMAGE_OUTPUT_PATH "output/connectionpin02-8");
