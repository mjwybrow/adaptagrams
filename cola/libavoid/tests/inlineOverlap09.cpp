...
int main(void) {
    ...
    router->setRoutingParameter(crossingPenalty, 100);
    router->setRoutingParameter(clusterCrossingPenalty, 4000);
    router->setRoutingParameter(fixedSharedPathPenalty, 1000);
    router->setRoutingParameter(portDirectionPenalty, 100);
    router->setRoutingParameter(shapeBufferDistance, 20);
    router->setRoutingParameter(idealNudgingDistance, 40);
    ...
    router->outputDiagram(IMAGE_OUTPUT_PATH "output/inlineOverlap09");
