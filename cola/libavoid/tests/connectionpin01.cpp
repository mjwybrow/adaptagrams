...
int main(void)
{
    ...
    router->outputDiagram(IMAGE_OUTPUT_PATH "output/connectionpin01-1");

    router->moveShape(shapeRef2, 5, 0);

    router->processTransaction();
    router->outputDiagram(IMAGE_OUTPUT_PATH "output/connectionpin01-2");
    
    router->moveShape(shapeRef1, 0, -10);

    router->processTransaction();
    router->outputDiagram(IMAGE_OUTPUT_PATH "output/connectionpin01-3");
